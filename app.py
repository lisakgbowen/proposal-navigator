...
.pn-spinner{
    ...
    height:76px;
    border-radius:50%;
    border:8px solid #fde1ea;
    border-top-color:var(--pink);
    animation:pnspin .9s linear infinite;
    display:flex;
    align-items:center;
    justify-content:center;
    color:var(--pink);
...
.pn-readiness{
    ...
    height:92px;
    border-radius:50%;
    background:conic-gradient(var(--pink) calc(var(--pct)*1%), #f8dce5 0);
    display:grid;
    place-items:center;
...
.pn-readiness-inner strong{
    display:block;
    font-size:1.25rem;
}
.pn-readiness-inner span{
    font-size:.61rem;
    color:var(--muted);
}
@media (prefers-reduced-motion: reduce){
    .pn-spinner{animation:none;}
}
...
with c2:
    with st.container(border=True):
        ...
        # Default all questions to "Not sure" so the progress bar starts at 0.
        personnel = st.radio(
            "Personnel costs?", ["Not sure", "Yes", "No"],
            horizontal=True, key="personnel"
        )
        travel = st.radio(
            "Travel?", ["Not sure", "Yes", "No"],
            horizontal=True, key="travel"
        )
        equipment = st.radio(
            "Equipment?", ["Not sure", "Yes", "No"],
            horizontal=True, key="equipment"
        )
        participant_incentives = st.radio(
            "Participant incentives?", ["Not sure", "Yes", "No"],
            horizontal=True, key="participant_incentives"
        )
        subawards = st.radio(
            "Subawards or contracts?", ["Not sure", "Yes", "No"],
            horizontal=True, key="subawards"
        )
        cost_share = st.radio(
            "Cost share or matching?", ["Not sure", "Yes", "No"],
            horizontal=True, key="cost_share"
        )
        indirect_costs = st.radio(
            "Indirect costs?", ["Not sure", "Yes", "No"],
            horizontal=True, key="indirect_costs"
        )
    ...
    with st.container(border=True):
        ...
        if st.session_state.review_complete:
            review_visual.markdown(
                ...
                'border:8px solid #dff4e9;display:flex;align-items:center;'
                'justify-content:center;color:#1e9b62;font-size:2rem;font-weight:700;">✓</div>'
                '</div>',
                unsafe_allow_html=True
            )
            review_status.markdown(
                '<div class="pn-status">✓ Review complete</div>',
                unsafe_allow_html=True
            )
        else:
            review_visual.markdown(
                '<div class="pn-mini"><b>Ready to analyze</b><br>'
                'Upload a funding opportunity, complete the guided intake, '
                'then run the review.</div>',
                unsafe_allow_html=True
            )
...
if run_review:
    ...
    # Live review animation and staged status updates.
    review_visual.markdown(
        '<div class="pn-review-wrap"><div class="pn-spinner">AI</div></div>',
        unsafe_allow_html=True
    )
    review_status.markdown(
    ...
    with st.spinner("Analyzing funding opportunity and generating readiness findings..."):
        ...
        for idx, (item, answer) in enumerate(responses.items(), start=1):
    ...
    review_status.markdown(
        ...
        '<div class="pn-status">✓ Matching proposal categories</div>'
        '<div class="pn-status">✓ Preserving source pages</div>'
        '<div class="pn-status">✓ Readiness report generated</div>',
        unsafe_allow_html=True
    )
    ...
    # SUMMARY ROW
    # -----------------------------
    # Simple v0 readiness percentage:
    # answered questions contribute 50%; source matches contribute 50%.
    answered_final = sum(1 for v in responses.values() if v != "Not sure")
    source_count = sum(1 for m in source_matches.values() if m["found"])
    readiness_pct = round(((answered_final / 7) * 50) + ((source_count / 7) * 50))
    ...
    with st.container(border=True):
        ...
        with circle_col:
            st.markdown(
                f'<div class="pn-readiness" style="--pct:{readiness_pct};position:relative;">'
                f'<div class="pn-readiness-inner"><strong>{readiness_pct}%</strong>'
    ...
    report_lines = [
        ...
        f"Clarifications needed: {len(missing)}",
    ...
    for finding in findings:
        report_lines.extend([
            ...
            f"Budget Impact: {finding['Budget Impact']}",
            f"Source: {finding['Source']}",
            f"Follow-Up: {finding['Follow-Up']}"
        ])
    ...
    report_lines.extend([
        "",
        "IMPORTANT",
        "Proposal Navigator is a decision-support prototype. "
        "Final proposal decisions require review of sponsor guidance, "
        "institutional policy, and Research Administration as applicable."
    ])
